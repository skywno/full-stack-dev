...
const getValidTokenFromTestUser = async () => {
  const response = await api.post("/api/login").send(testUser[0]).expect(200);
  return response.body.token;
};

const createTestUser = async () => {
  await api.post("/api/users").send(testUser[0]).expect(200);
};
...
describe(`[DELETE] /api/blogs/{id}`, () => {
  ...
  test("Given an invalid id despite a valid token, then fails to delete the blog", async () => {
    const token = await getValidTokenFromTestUser();
    await api
      .delete("/api/blogs/32456786543245")
      .set({ Authorization: `Bearer ${token}` })
      .expect(404);
  });
});
